...
def main():
    ...
    net = ResNet14(wide=1,first_layer=64,samesize=True)
#    net = torch.load("checkpoint/11010_teacher_145_student.pth")["net"].module
    net = net.to(device)
    teacher = torch.load("checkpoint/resnet11010-2.pth")["net"]
    teacher.eval()
    ...
    for epoch in range(250):
        ...
        test(net,testloader, device, "11010_teacher_144_samesize_student")
    optimizer = optim.SGD(net.parameters(), lr=0.001, momentum=0.9, weight_decay=1e-4)
    scheduler = MultiStepLR(optimizer, milestones=[100,175], gamma=0.1)
    for epoch in range(5):
        print('\nEpoch: %d' % epoch)
        train(net,clean_trainloader,scheduler, device, optimizer, teacher=teacher, mixup_alpha=0)
        test(net,testloader, device, "11010_teacher_144_samesize_student-2")
