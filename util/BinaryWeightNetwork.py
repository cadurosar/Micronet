...
class BinaryWeightNetwork():
    def __init__(self, model:nn.Module, BWN=True):
        """
    ...
    def binarize(self, determinist = True):
        ...
        for i in range(self.params):
            ...
            if self.BWN and true_value.dim() == 4: # if BWN and convolution
                with torch.no_grad():
                    filter_avg = torch.mean(torch.abs(true_value), (1, 2, 3))
                    for j in range(quantized.size(0) - 1):
                        quantized[j,:,:,:] *= filter_avg[j]
